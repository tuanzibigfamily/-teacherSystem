...
public class LoginFrame extends JFrame {
    ...
    // 定义图标
    private ImageIcon iconShow = new ImageIcon("O:\\Shixi_Shixi_Shixi\\teacher-workload-system\\image\\显示密码.png");  // 显示密码图标路径
    ...
    private ImageIcon iconHide = new ImageIcon("O:\\Shixi_Shixi_Shixi\\teacher-workload-system\\image\\隐藏密码.png"); // 隐藏密码图标路径
...
// 设置按钮格式
    ...
    // 弹出反馈窗口
    private void showFeedbackWindow(String title, String message) {
        JFrame feedbackFrame = new JFrame(title);
        feedbackFrame.setSize(300, 150);
        feedbackFrame.setLocationRelativeTo(this);  // 使反馈窗口相对于当前窗口居中
        feedbackFrame.setDefaultCloseOperation(JFrame.DISPOSE_ON_CLOSE);

        JLabel feedbackLabel = new JLabel(message, SwingConstants.CENTER);
        feedbackLabel.setFont(new Font("微软雅黑", Font.PLAIN, 16));
        feedbackLabel.setPreferredSize(new Dimension(280, 50));

        JButton okButton = new JButton("确定");
        okButton.setFont(new Font("微软雅黑", Font.PLAIN, 14));
        okButton.addActionListener(e -> feedbackFrame.dispose());

        feedbackFrame.setLayout(new BorderLayout());
        feedbackFrame.add(feedbackLabel, BorderLayout.CENTER);
        feedbackFrame.add(okButton, BorderLayout.SOUTH);
    ...
    private void togglePasswordVisibility() {
        ...
        if (isPasswordVisible) {
            ...
            timer.schedule(new TimerTask() {
                @Override
                public void run() {
                    passwordField.setEchoChar('*');  // 恢复为圆点格式
...
// 显示注册成功的信息对话框
    ...
    private class BackgroundPanel extends JPanel {
        ...
        public BackgroundPanel() {
            backgroundImage = new ImageIcon("O:\\Shixi_Shixi_Shixi\\teacher-workload-system\\image\\登gss录.jpg"); // 这里的"background.jpg"是你的背景图片路径
